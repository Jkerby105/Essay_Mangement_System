      ...
      if (essayValue !== "null") {
        try {
          const response = await fetch(
            "http://localhost:3000/essay/oneEssay/" + essayValue
          );
...
export async function action({ request, params }) {
  ...
  if (data.get("delete") === "delete") {
    response = await fetch("http://localhost:3000/essay/delete/" + essayID, {
      method: "DELETE",
      headers: {
  ...
  } else if (data.get("update") === "true") {
    response = await fetch(
      "http://localhost:3000/essay/essayUpdate/" + essayID,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
